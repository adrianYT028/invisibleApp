...
CapturedImage ScreenCapture::CaptureRegion(const Rect& region) {
    ...
    CapturedImage result = CaptureFromDC(screenDC, region.x, region.y, region.width, region.height);
...
CapturedImage ScreenCapture::CaptureFromDC(HDC srcDC, int srcX, int srcY, int width, int height) {
    CapturedImage image;
    ...
    // This is intentional - it's how we ensure our overlay doesn't appear in captures
...
bool ScreenCapture::SaveToBmp(const CapturedImage& image, const wchar_t* filePath) {
    if (!image.IsValid()) {
        return false;
    }
...
bool ScreenCapture::SaveToPpm(const CapturedImage& image, const wchar_t* filePath) {
    if (!image.IsValid()) {
        return false;
    }
...
void RegionSelector::StartSelection(SelectionCallback callback) {
    ...
    SetWindowLongPtrW(selectorHwnd_, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(this));
    ...
    if (screenSnapshot_.IsValid()) {
        ...
        snapshotBitmap_ = CreateDIBitmap(
            screenDC,
            &bmi.bmiHeader,
            CBM_INIT,
            screenSnapshot_.pixels.data(),
            &bmi,
            DIB_RGB_COLORS
        );
...
LRESULT CALLBACK RegionSelector::SelectorWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    RegionSelector* self = reinterpret_cast<RegionSelector*>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
...
LRESULT RegionSelector::HandleSelectorMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_PAINT: {
            PAINTSTRUCT ps;
            ...
            if (snapshotBitmap_) {
                ...
                HBITMAP oldBmp = static_cast<HBITMAP>(SelectObject(memDC, snapshotBitmap_));
                
                // Adjust for virtual screen offset
                Rect virtualScreen = GetVirtualScreenRect();
                BitBlt(hdc, 0, 0, rc.right, rc.bottom, 
                       memDC, -virtualScreen.x, -virtualScreen.y, SRCCOPY);
            ...
            if (isDragging_) {
                ...
                if (sel.width > 0 && sel.height > 0) {
                    ...
                    RECT textRect = {adjX + 5, adjY + sel.height + 5, adjX + 150, adjY + sel.height + 25};
